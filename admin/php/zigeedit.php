...
$id = $_POST['id'];
$text = $_POST['content'];
if (empty($_FILES['zigebook']['size'])) {
	$sql=JXMySQL_Execute("update `shop_certificate` set text = ? where id = ?",array($text,$id));
	...
	$img = $_FILES['zigebook'];//获取到表单过来的文件变量，uploadImg为表单id
	if(isset($img))
	{
		...
		if(strtolower($type)=='.png'||strtolower($type)=='.jpg'||strtolower($type)=='.bmp'||strtolower($type)=='.gif'||strtolower($type)=='.jpeg')//判断上传的文件是否为图片格式
		{
			...
			$url=$path;
			$sql=JXMySQL_Execute("update `shop_certificate` set text = ?,pic = ? where id = ?",array($text,$url,$id));
