...
//上传成功$img中的属性error为0，当error>0时则上传失败有一下几种情况
    if($img['error']>0){
        //echo "上传失败";
        $sql=JXMySQL_Execute("update `shop_huxing` set info = '$articletitle' where id= '$id'");
        ...
        if(strtolower($type)=='.png'||strtolower($type)=='.jpg'||strtolower($type)=='.bmp'||strtolower($type)=='.gif'||strtolower($type)=='.jpeg')//判断上传的文件是否为图片格式
        {
            ...
            // echo $path;
            $url=$path;
            $sql=JXMySQL_Execute("update `shop_huxing` set pig= ?,info = ? where id= ?",array($url,$articletitle,$id));
            $insert_id = JXMySQL_Insert($sql);
            if ($insert_id) {
              echo  false;
