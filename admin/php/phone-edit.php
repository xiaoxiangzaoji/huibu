...
if (empty($_FILES['photo']['size'])) {	
		header("Location:../html/phone-banner.php");  
		exit();
}else{
	...
	if(isset($img))
	{
		...
		$path = "../html/temp/phone/".$img['name'];//设置路径：当前目录下的uploads文件夹并且图片名称为$img['name'];
		if(strtolower($type)=='.png'||strtolower($type)=='.jpg'||strtolower($type)=='.bmp'||strtolower($type)=='.gif'||strtolower($type)=='.jpeg')//判断上传的文件是否为图片格式
		{
			...
			$url=$path;
			$sql=JXMySQL_Execute("update `phone_banner` set picture = ? where id = ?",array($url,$id));
