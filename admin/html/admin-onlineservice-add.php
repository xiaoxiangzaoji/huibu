...
<article class="page-container">
	<form action="" method="post" class="form form-horizontal" id="form-admin-role-add">
		...
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>客服信息：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" placeholder="" id="info" name="info" >
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>对话链接：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" placeholder="" id="contact" name="contact" style="width:300px;">
			</div>
		</div>
		<div class="row cl">
...
$(function(){
	...
	$('.btn').bind('click',function(){
		...
		var info=$('#info').val();
		if(contact==''){
			$('#contact').attr('placeholder','内容不能为空！');
		}
		if(!contact == ''){
			var pattern=/^\d+(\.\d+)?$/;
			var flag = pattern.test(contact);
		}
		if(!flag){
		...
		if(roleName!='' && contact!=''){
			$.ajax({
				type:"post",
				url:"/cms/admin/php/admin-onlineservice-add.php",
				async:true,
				dataType:"json",
				data:{
					roleName:roleName,
				...
				success:function(data){
					...
					if(data.code==1){
						...
						layer.msg('已经存在了!',{icon:5,time:1000});
					}
				}
			});
		}
