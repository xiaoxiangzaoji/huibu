...
<article class="page-container">
   ...
   <?php
                if(!$_GET["id"]){
                 ...
                 $sql="select * from phone_data where id='{$ID}'";
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="<?php echo $v['title'] ?>" placeholder="" id="adminName" name="adminName">
		</div>
	</div>
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3">作者：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="<?php echo $v['author'] ?>" placeholder="" id="web" name="web">
		</div>
	</div>
	<div class="row cl">
...
var ue = UE.getEditor('container');
	$('input').eq(2).bind('click',function(){
			...
			var name = $('#adminName').val();//标题
			var web = $('#web').val();//作者
			var info = ue.getContent();//内容
			var id=getUrlParam('id');
		$.ajax({

			type:"post",
			url:"/cms/admin/php/phonedata-edit.php",
			async:true,
			dataType:"json",
			data:{
                id :id,
