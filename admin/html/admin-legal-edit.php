...
<article class="page-container">
   ...
   <?php
                if(!$_GET["id"]){
                 ...
                 $sql="select * from shop_purchase where id='{$ID}'";
      ...
      ?>
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3">法律法规：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="<?php echo $v['text'] ?>" placeholder="" id="adminName" name="adminName">
		</div>
	</div>
	<div class="row cl">
...
$(function(){
	...
	function refer(){  
	    if(t==0){ 
	        window.location.href="admin-legal.php";  //#设定跳转的链接地址 
	    }else{
	   		t--; // 计数器递减 
		}
	} 
...
var ue = UE.getEditor('container');
	$('input').eq(1).bind('click',function(){
		...
		$.ajax({
			...
			type:"post",
			url:"/cms/admin/php/shop_information-edit.php",
			async:true,
			dataType:"json",
			data:{
                id :id,
